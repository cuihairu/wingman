...
## 17. 会话审计报表（P1 落地，2026-09-25）

### 为什么另立专表而不是复用 AuditLog

`AuditLog` 是**事件流水**（append-only，一条事件一行，meta 是 JSON），回答
「某人在某时刻做了什么」，给合规逐条查。报表要回答的是另一类问题：
「这台机器这个月被谁接管了多久、录了几段、失败几次」——那需要**可聚合的
结构化列**（协议 / 模式 / 时长 / 录像名）。在 JSON meta 上做聚合既慢又脆
（`json_extract` 无法走索引，且 meta 结构会随事件演进漂移）。

故新增 `models.RemoteSessionAudit`（表 `remote_session_audits`）：既有
`AuditLog` 的四事件审计（票据/连接/断开/失败）**保持不变**——两者受众不同，
缺一不可。

### 落库时机：会话结束时一次性写终态行

只写终态（`closed` / `failed`），进行中的会话不落行。理由：

- 报表若把「还没结束的会话」算进时长与计数，数字会持续漂移；
- 进程崩溃会留下永远不闭合的脏行（`ended_at` 为空、时长为 0），无法与
  真实零时长会话区分。

代价：「当前有多少人在看」无法从本报表得到——那是**实时态**，属 registry 的
职责，不属审计。

`RecordRemoteSession` 是唯一落库入口：UTC 归一、时长口径、终态枚举都是审计
契约的一部分，散在调用点迟早分叉。落库失败只记日志不阻断会话关闭（审计写不
进去不该让用户的会话卡住）。

### 录像可关联

`recording_name` 与录像检索 API 的 `name` 同值（`{agentID}-{sessionID}.mjs`），
报表点「有录像」即可跳到该录像。会话 ID 提前到拨号前生成，故**建连失败也有
唯一标识**（否则报表只能看到「有一次失败」而无法定位是哪次）。

### 查询接口

```
GET /api/remote/sessions            desktop:view（只读，报表不含接管能力）
  ?page&size(≤200)                  分页
  ?agentId&protocol&operator&status 多维过滤
  ?mode=view|control                监看/接管（UI 语义 → read_only 布尔列）
  ?record=true                      只看有录像的会话
  ?start&end                        RFC3339 时间范围
  ?groupBy=protocol|operator|agentId 维度聚合（白名单化，杜绝 SQL 注入）
  ?bucket=hour|day|week|month       时间趋势分桶
```

一次请求同时返回**列表 + 汇总 + 维度聚合 + 时间趋势**四块视图。分四个端点
迟早出现「列表 12 条、汇总 13 条」的对不上——报表最常见的错误就是口径分叉。
汇总/分组/分桶都基于**全量过滤结果**（不受分页影响），且四条查询共用同一个
过滤器构造函数。

### 安全与权限

- 不含任何凭证与画面内容（密码只在网关→guacd 的 connect 指令里，从不落库）；
- 权限 `desktop:view`——与录像检索同级。**不新增 RBAC 码**：报表是只读视图，
  监看者能看报表不该比看画面需要更多权限；
- `host`/`port` 属内网 endpoint 信息，与既有 `desktop.*` 审计同源同权限。

### 已知取舍

- 无保留/清理策略：会话审计行数增长很慢（远控是低频操作），第一版不做 TTL；
  真到需要时按 `started_at` 索引清理即可。
- 无跨实例聚合：报表是单库聚合。多节点部署需要各节点库汇总，属部署形态
  确定后再设计的问题。
...
### 15.1 SSH/SFTP 文件浏览器（第一版：浏览/下载/上传，2026-09-26 实现）
...
### 15.2 文件浏览器第二版（2026-09-26 实现）：分页/进度/重试/审计

在 v1 协议编排之上收口四件事，全部在「协议内可做」的边界里；协议做不到的
（服务端分页、删除/重命名）不硬做：

- **大目录**：协议一次 `get` 返回整个目录体，**没有服务端分页**——分页只
  能做在浏览器侧（antd Table 每页 50 行，小列表自动隐藏翻页；排序维持
  目录在前、名称升序）。目录体聚合加 8 MiB 护栏：超限拒绝并停止 ack
  （ack 纪律即流控，guacd 自然停发），防异常目录打爆内存。
- **进度**：下载逐块上报已收字节（总量取 `entry.size` 折百分比）；上传走
  `BlobWriter.onprogress`（1.5.0 实测存在：逐 ack 触发，第二参为已发字节）。
- **失败重试**：`withRetry`（默认 2 次尝试、按序退避），重试耗尽才报错，
  实际尝试次数进审计。**关键协议修正**：1.5.0 `BlobWriter` 对错误 ack 只
  停发不报错（`onerror` 仅本地读文件失败触发，dist/esm 实测佐证），v1 的
  上传失败路径是死代码——v2 从 `writer.onack`（`status.code !== 0`）判定
  失败；再叠「无进展看门狗」（默认 60s，可注入，0 关闭）兜底协议静默。
  下载侧协议无错误 ack 可判，停滞检测同样靠看门狗（超时即 reject 可重试）。
- **文件操作审计**：下载/上传的最终结果（ok/fail、路径、字节数、尝试次数）
  POST `/api/remote/file-ops` 落审计（`desktop.file_download`/
  `desktop.file_upload`）。权限：路由组 desktop:view；upload 是写动作，
  handler 内联追加 desktop:control 校验——「监看可浏览/下载、上传仅接管」
  的矩阵在审计通道同样成立（view 身份伪造 upload 上报也是 403）。
  **信任模型**：上报是客户端 best-effort，会话/agent/操作者一律由服务端
  按票据反解（网关 WS 建连后存「票据→会话快照」，closeSession 清除；
  请求体同类字段不采信）；票据反解不到（进程重启/会话已关）落
  `no_session` 降级行而非拒绝——上报本身仍值得留痕。列表**不审计**（高频，
  与 agents 列表同策略）；删除/重命名审计随协议能力一并不存在（§15.1）。
