...
- [x] C++ 第五批补测（2026-09-22）：script_module 胶水全链 7 用例（真实 ScriptManager 注入，12 函数此前零覆盖——15.5% 全库最低，含 null 注入与参数不足防御全触达；实测状态机 load→unloaded/run→completed/reload→loaded）+ filewatcher stub 1 + clipboard 胶水 2（LockGuard 串行化）+ verification TOTP 3（Base32 校验链/自产自销 verify）+ ml stub 错误分支 1 + game_profile 3（临时目录真实落盘：模板/JSON 往返/删除幂等）。**85.4% → 86.8% 行（13092 行，v9 终版基线，全量 1909 用例）**。不可覆盖论证：filewatcher.watch 成功行（ScriptValue 无公开 Callable 构造器）、ml 真实模型路径（CI 为 ml_stub）。
- [ ] C++ 生产代码继续补测（剩余缺口见 v8 基线明细）。预期剩余：misc_modules UIA 注册表段（Windows 专属，Linux 不可测）、ml/game_profile/notify 胶水错误分支、x11_recorder XRecord 正向链路（需真实桌面，已由 verify-xrecord-desktop.sh 真机脚本覆盖）、Windows/macOS 平台实现（真机专属，逐条论证不硬凑）。
