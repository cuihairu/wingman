...
- [ ] C++ 生产代码继续补测（剩余缺口见 v11 基线明细）：x11_recorder 67.3%（91 行，XRecord 正向链路需真实桌面，已由 verify-xrecord-desktop.sh 真机脚本覆盖）、clipboard.cpp 58.1%（26 行）、ml_module 67.3%（34 行）、x11_clipboard 71.8%（37 行）、debugger/orchestration/security 胶水零星段、crypt 72.0% 剩余 77 行（OpenSSL 内部失败防御，不可确定性触发，论证在案）、Windows/macOS 平台实现（真机专属，逐条论证不硬凑）。
