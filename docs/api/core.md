...
| 模块 | 功能 | 文档 |
|------|------|------|
| `screen` | 屏幕操作 | [#screen-屏幕操作](#screen-屏幕操作) |
| `input` | 输入模拟 | [#input-输入模拟](#input-输入模拟) |
| `window` | 窗口管理 | [#window-窗口管理](#window-窗口管理) |
| `process` | 进程管理 | [#process-进程管理](#process-进程管理) |
| `trigger` | 触发器系统 | [#trigger-触发器系统](#trigger-触发器系统) |
| `vision` | 图像识别 | [#vision-图像识别](#vision-图像识别) |
| `ocr` | OCR 文字识别 | [#ocr-文字识别](#ocr-文字识别) |
| `clipboard` | 剪贴板操作 | [#clipboard-剪贴板](#clipboard-剪贴板) |
| `recorder` | 宏录制 | [#recorder-宏录制](#recorder-宏录制) |
...
```lua
{
    type = "color",           -- 触发器类型: "color", "image", "time", "key"
    enabled = true,            -- 是否启用
    ...
    -- 回调函数
    action = function(match)   -- 触发时执行的操作
        print("Triggered!", match.x, match.y)
    end
}
```
...
-- Lua
-- 颜色触发器
local colorTrigger = trigger.create({
    type = "color",
    color = 0xFF0000,
    x = 0, y = 0,
    width = 1920, height = 1080,
    ...
    action = function(match)
        print("Found red at:", match.x, match.y)
        input.click(match.x, match.y, "left")
    end
})

-- 图像触发器
local imageTrigger = trigger.create({
    type = "image",
    image = "enemy.png",
    x = 0, y = 0,
    width = 1920, height = 1080,
    threshold = 0.85,
    action = function(match)
        print("Found enemy at:", match.x, match.y)
        input.keyPress("SPACE")
    end
})

-- 时间触发器
local timerTrigger = trigger.create({
    type = "time",
    interval = 5000,
    action = function()
        print("Timer triggered every 5 seconds")
    end
})
```
...
color_trigger = trigger.create({
    ...
    "tolerance": 10,
    "action": lambda match: (
        print(f"Found red at: {match['x']}, {match['y']}"),
        input.click(match['x'], match['y'], "left")
    )
})
```
...
```lua
-- Lua
if trigger.isEnabled(colorTrigger) then
...
```lua
local clipboard = require("wingman.clipboard")
```
...
宏录制模块提供录制和回放鼠标键盘操作的功能。

### Lua API

```lua
local recorder = require("wingman.recorder")
```

### Python API

```python
from wingman import recorder
```
...
**参数:**
- `config` (table/object): 录制配置

**配置选项:**
```lua
{
    recordMouse = true,      -- 录制鼠标操作
    recordKeyboard = true,    -- 录制键盘操作
    recordDelay = true,       -- 录制延迟
    filterDuplicates = true   -- 过滤重复操作
}
```
...
```lua
-- Lua
local data = recorder.stop()
print("Recorded " .. #data .. " actions")
```

```python
# Python
data = recorder.stop()
print(f"Recorded {len(data)} actions")
```
...
-- Lua
local data = recorder.stop()
recorder.save(data, "recording.json")
```
...
```lua
-- Lua
local data = recorder.load("recording.json")
```
