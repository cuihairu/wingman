...
ModuleDescriptor createProcessModule() {
	...
	mod.functions.push_back({"wait", [](const std::vector<ScriptValue>& args) -> ScriptValue {
		ProcessId pid = static_cast<ProcessId>(args[0].asInt());
		int timeout = args.size() > 1 ? static_cast<int>(args[1].asInt(0)) : 0;
	...
	mod.functions.push_back({"exists", [](const std::vector<ScriptValue>& args) -> ScriptValue {
		return ScriptValue::fromBool(Process::exists(static_cast<ProcessId>(args[0].asInt())));
	}, "pid:int -> bool"});
