...
-- 3. 反调试检测
print("3. 反调试检测")
local hasDebugger = security.isDebuggerPresent()
print(string.format("调试器检测: %s", hasDebugger and "发现调试器!" or "安全"))
print()

-- 4. 虚拟机检测
print("4. 虚拟机检测")
local inVM = security.isRunningInVM()
print(string.format("虚拟机环境: %s", inVM and "在虚拟机中运行" or "物理机"))
print()

-- 5. 完整性验证
print("5. 程序完整性验证")
local integrity = security.verifyIntegrity()
print(string.format("完整性: %s", integrity and "通过" else "失败"))
print()

-- 6. 字符串哈希
print("6. 字符串哈希")
local hash = security.hashString("Hello, Wingman!")
...
local encrypted = security.encryptString(message, secret)
print(string.format("原文: %s", message))
print(string.format("加密: %s", encrypted))
local decrypted = security.decryptString(encrypted, secret)
...
local filtered = security.filterSensitive(log)
...
print([[
    ...
    -- 执行点击
    input.click(clickX, clickY, "left")
]])
print()
