...
//
//	uint32 length + uint32 sequence + uint8 type + 3 字节 padding + uint32 reserved
const (
	frameHeaderSize = 16
	frameRequest    = byte(1)
...
// mockCommand 记录 orchestrator 下发给 agent 的一条命令。
...
// mockAgent 模拟 C++ runtime agent：outbound 连接 FrameListener，讲相同的帧协议，
// 替代真实 C++ runtime。readLoop 是唯一读方；收到 Request 帧后回调 handler 并回写
// Response 帧（命令结果回传）。
...
// newMockAgent 连接 orchestrator、发送 agent.register 并等待 register_ack。
func newMockAgent(t *testing.T, addr, agentID, hostname string, handler func(string, map[string]any) map[string]any) *mockAgent {
	t.Helper()
	conn, err := net.DialTimeout("tcp", addr, 2*time.Second)
	if err != nil {
		t.Fatalf("mock agent %s dial %s: %v", agentID, addr, err)
	}
	a := &mockAgent{t: t, id: agentID, conn: conn, handler: handler}
	t.Cleanup(a.close)
	go a.readLoop()
...
func (a *mockAgent) writeFrame(msgType byte, seq uint32, payload any) {
	body, err := json.Marshal(payload)
	if err != nil {
		a.t.Errorf("mock agent %s marshal frame: %v", a.id, err)
		return
	}
	a.wmu.Lock()
	defer a.wmu.Unlock()
...
func (a *mockAgent) notify(msgType string, extra map[string]any) {
...
// handleRequest 处理服务端下发的命令：记录命令后回写 Response 帧（结果回传）。
func (a *mockAgent) handleRequest(seq uint32, body []byte) {
	...
	a.mu.Lock()
	a.commands = append(a.commands, mockCommand{Method: method, Seq: seq, Data: msg, At: time.Now()})
	handler := a.handler
	a.mu.Unlock()
...
func (a *mockAgent) recordNotify(body []byte) {
	if string(body) == "PONG" {
		a.mu.Lock()
...
func (a *mockAgent) heartbeat(status string, resources map[string]any) {
...
func (a *mockAgent) setHandler(h func(method string, data map[string]any) map[string]any) {
	a.mu.Lock()
	defer a.mu.Unlock()
	a.handler = h
}

func (a *mockAgent) commandsOf(method string) []mockCommand {
	a.mu.Lock()
	defer a.mu.Unlock()
	var out []mockCommand
...
func (a *mockAgent) waitForCommands(t *testing.T, method string, n int, timeout time.Duration) []mockCommand {
	t.Helper()
	var out []mockCommand
...
func (a *mockAgent) waitForNotify(t *testing.T, typ string, timeout time.Duration) {
	t.Helper()
